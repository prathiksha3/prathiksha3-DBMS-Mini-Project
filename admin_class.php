...
Class Action {
    ...
    extract($_POST);
    // Get the details of the tenant before deletion
    $tenant_details_query = $this->db->query("SELECT * FROM tenants WHERE id = $id");
    $tenant_details = $tenant_details_query->fetch_assoc();
    
    // Delete the tenant from the tenants table
    $delete_tenant_query = $this->db->query("DELETE FROM tenants WHERE id = $id");
    
    if($delete_tenant_query){
        // Insert the tenant details into the deleted_tenants table
        $insert_query = $this->db->query("INSERT INTO deleted_tenants (id, firstname, lastname, middlename, email, contact, house_id, date_in) 
                                            VALUES ('".$tenant_details['id']."', '".$tenant_details['firstname']."', '".$tenant_details['lastname']."', 
                                            '".$tenant_details['middlename']."', '".$tenant_details['email']."', '".$tenant_details['contact']."', 
                                            '".$tenant_details['house_id']."', '".$tenant_details['date_in']."')");
        if($insert_query){
            return 1; // Return success if deletion and insertion are successful
        } else {
